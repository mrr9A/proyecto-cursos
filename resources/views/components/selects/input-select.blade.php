<div class="text-text-input relative">
    ...
    <select {{ $required ?? '' ? 'required' : '' }} name="{{ $name }}" id="{{ $id ?? $name }}"
        class="bg-gray-50 border-[2px] border-input text-gray-900  rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white {{$class ?? ""}}">
        <option value="" id="select_puesto" class="text-gray-400" {{ $disabled ?? '' ? 'disabled' : '' }} >{{ $textOptionDefault }}</option>
        @if ($puestos ?? '')
