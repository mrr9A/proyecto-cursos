...
<div id="dropdownInformation" class="z-10 hidden text-white bg-primary divide-y divide-gray-100 rounded-lg shadow-lg w-52 dark:bg-gray-700 dark:divide-gray-600">
    ...
    <ul class="py-2 text-sm dark:text-gray-200" aria-labelledby="dropdownInformationButton">
        <li>
            <a href="#" class="block px-4 py-2 hover:bg-gray-600 dark:hover:bg-gray-600 dark:hover:text-white">Configuraciones</a>
        </li>
    </ul>
    <div class="py-2">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="block px-4 py-2 text-sm hover:bg-gray-600 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Cerrar sesión</button>
        </form>
    </div>
</div>