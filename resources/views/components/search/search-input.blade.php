<form autocomplete="off" class="flex items-center w-[400px] relative" action="{{ route('cursos.index') }}">
    <label for="simple-search" class="sr-only">Search</label>
    <div class="relative w-full">
        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            <svg aria-hidden="true" class="w-5 h-5 text-white dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20"
...
<script>
    ...
    buscador.addEventListener('keyup', (e) => {
        debounce(getInfoByBuscador(e.target.value), 3000)
    })
        ...
        fetch(`http://localhost:8000/api/buscador?buscar=${texto}`)
            ...
            .then(data => {
                ...
                data.forEach(element => {
                    list += `
                    <li class="list">${element.nombre}</li>
                    `
                });
